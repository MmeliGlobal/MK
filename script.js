...
// ==================== QUOTATION FUNCTIONS ====================
function setupQuotationForm() { /* your existing code – unchanged */ const today = new Date().toISOString().split('T')[0]; const issueDateInput = document.getElementById("quoteIssueDate"); if (issueDateInput) issueDateInput.value = today; const validThroughInput = document.getElementById("quoteValidThrough"); if (validThroughInput) { const future = new Date(); future.setDate(future.getDate() + 30); validThroughInput.value = future.toISOString().split('T')[0]; } const addBtn = document.getElementById("addItemBtn"); if (addBtn) addBtn.addEventListener("click", addItemRow); const container = document.getElementById("quoteItemsContainer"); if (container) { container.addEventListener("input", calculateQuoteTotals); const discountValue = document.getElementById("discountValue"); if (discountValue) discountValue.addEventListener("input", calculateQuoteTotals); const taxRate = document.getElementById("taxRate"); if (taxRate) taxRate.addEventListener("input", calculateQuoteTotals); const discountType = document.getElementById("discountType"); if (discountType) discountType.addEventListener("change", calculateQuoteTotals); calculateQuoteTotals(); } }
function addItemRow() { const container = document.getElementById("quoteItemsContainer"); if (!container) return; const row = document.createElement("div"); row.className = "quote-item-row"; row.innerHTML = `<input type="text" class="item-desc" placeholder="Description" style="width:40%"><input type="number" class="item-qty" placeholder="Qty" value="1" style="width:15%"><input type="number" class="item-price" placeholder="Unit Price" style="width:20%"><span class="item-subtotal">0.00</span><button type="button" class="remove-item" style="width:10%">✖</button>`; row.querySelector(".remove-item").addEventListener("click", () => row.remove()); container.appendChild(row); calculateQuoteTotals(); }
function calculateQuoteTotals() { let subtotal = 0; const rows = document.querySelectorAll("#quoteItemsContainer .quote-item-row"); rows.forEach(row => { const qty = parseFloat(row.querySelector(".item-qty").value) || 0; const price = parseFloat(row.querySelector(".item-price").value) || 0; const subtotalRow = qty * price; row.querySelector(".item-subtotal").innerText = subtotalRow.toFixed(2); subtotal += subtotalRow; }); const discountType = document.getElementById("discountType").value; let discountValue = parseFloat(document.getElementById("discountValue").value) || 0; let discountAmount = discountType === "percentage" ? (discountValue / 100) * subtotal : discountValue; const afterDiscount = subtotal - discountAmount; const taxRate = parseFloat(document.getElementById("taxRate").value) || 0; const taxAmount = (taxRate / 100) * afterDiscount; const total = afterDiscount + taxAmount; document.getElementById("subtotal").innerText = subtotal.toFixed(2); document.getElementById("discountAmount").innerText = discountAmount.toFixed(2); document.getElementById("taxAmount").innerText = taxAmount.toFixed(2); document.getElementById("totalAmount").innerText = total.toFixed(2); }
function generateQuote() { const quoteData = { id: Date.now(), quoteNumber: document.getElementById("quoteNumber").value.trim() || "QT" + Math.floor(Math.random() * 10000), issueDate: document.getElementById("quoteIssueDate").value, validThrough: document.getElementById("quoteValidThrough").value, company: { name: document.getElementById("quoteCompanyName").value, address1: document.getElementById("quoteCompanyAddress").value, address2: document.getElementById("quoteCompanyAddress2").value, address3: document.getElementById("quoteCompanyAddress3").value, phone: document.getElementById("quoteCompanyPhone").value }, client: { name: document.getElementById("quoteClientName").value, phone: document.getElementById("quoteClientPhone").value, address1: document.getElementById("quoteClientAddress1").value, address2: document.getElementById("quoteClientAddress2").value, address3: document.getElementById("quoteClientAddress3").value }, items: [], discountType: document.getElementById("discountType").value, discountValue: parseFloat(document.getElementById("discountValue").value) || 0, taxRate: parseFloat(document.getElementById("taxRate").value) || 0, notes: document.getElementById("quoteNotes").value }; const rows = document.querySelectorAll("#quoteItemsContainer .quote-item-row"); rows.forEach(row => { const desc = row.querySelector(".item-desc").value; const qty = parseFloat(row.querySelector(".item-qty").value) || 0; const price = parseFloat(row.querySelector(".item-price").value) || 0; if (desc && qty > 0 && price > 0) quoteData.items.push({ desc, qty, price, subtotal: qty * price }); }); if (quoteData.items.length === 0) { alert("Add at least one item."); return; } let subtotal = quoteData.items.reduce((sum, i) => sum + i.subtotal, 0); let discountAmount = quoteData.discountType === "percentage" ? (quoteData.discountValue / 100) * subtotal : quoteData.discountValue; let afterDiscount = subtotal - discountAmount; let taxAmount = (quoteData.taxRate / 100) * afterDiscount; quoteData.subtotal = subtotal; quoteData.discountAmount = discountAmount; quoteData.taxAmount = taxAmount; quoteData.total = afterDiscount + taxAmount; let quotationsArray = JSON.parse(localStorage.getItem('quotations') || '[]'); quotationsArray.push(quoteData); localStorage.setItem('quotations', JSON.stringify(quotationsArray)); alert("Quote saved!"); viewQuote(quoteData.id); }
function viewQuote(quoteId) { let quotationsArray = JSON.parse(localStorage.getItem('quotations') || '[]'); const quote = quotationsArray.find(q => q.id == quoteId); if (!quote) return; let html = `<div style="font-family: monospace; max-width: 800px; margin:0 auto;"><h2>${escapeHtml(quote.company.name)}</h2><div>${escapeHtml(quote.company.address1)}</div><div>${escapeHtml(quote.company.address2)}</div><div>${escapeHtml(quote.company.address3)}</div><div>${escapeHtml(quote.company.phone)}</div><hr><h3>RECEIVER</h3><div>${escapeHtml(quote.client.name)}</div><div>${escapeHtml(quote.client.address1)}</div><div>${escapeHtml(quote.client.address2)}</div><div>${escapeHtml(quote.client.address3)}</div><hr><h3>Logistics Quote</h3><p>Quote Number: ${escapeHtml(quote.quoteNumber)}</p><p>Issue Date: ${new Date(quote.issueDate).toLocaleDateString()}</p><p>Valid Through: ${new Date(quote.validThrough).toLocaleDateString()}</p><hr><h4>PAYMENT CONDITIONS</h4><table border="1" cellpadding="5" cellspacing="0" style="width:100%; border-collapse: collapse;"><thead><tr><th>DESCRIPTION</th><th>QTY</th><th>UNIT PRICE</th><th>SUBTOTAL</th><th>TAX</th></tr></thead><tbody>`; quote.items.forEach(item => { const itemTax = (item.subtotal / quote.subtotal) * quote.taxAmount; html += `<tr><td>${escapeHtml(item.desc)}</td><td>${item.qty}</td><td>$${item.price.toFixed(2)}</td><td>$${item.subtotal.toFixed(2)}</td><td>$${itemTax.toFixed(2)}</td></tr>`; }); html += `</tbody></table>`; if (quote.notes) html += `<p><em>${escapeHtml(quote.notes)}</em></p>`; html += `<hr><p>Subtotal: $${quote.subtotal.toFixed(2)}</p><p>Discount (${quote.discountType === 'percentage' ? quote.discountValue + '%' : '$' + quote.discountValue}): -$${quote.discountAmount.toFixed(2)}</p><p>Tax (${quote.taxRate}%): $${quote.taxAmount.toFixed(2)}</p><p><strong>Total: $${quote.total.toFixed(2)}</strong></p></div>`; document.getElementById("quotePreview").innerHTML = html; document.getElementById("quoteModal").style.display = "flex"; }
function closeQuoteModal() { document.getElementById("quoteModal").style.display = "none"; }
function printQuote() { const printContents = document.getElementById("quotePreview").innerHTML; const printWindow = window.open('', '_blank'); printWindow.document.write(`<html><head><title>Quote</title><style>body { font-family: monospace; padding: 20px; } table { width:100%; border-collapse: collapse; } th, td { border: 1px solid #000; padding: 8px; text-align: left; }</style></head><body>${printContents}</body></html>`); printWindow.document.close(); printWindow.print(); }

// ==================== SHIPPING FUNCTIONS ====================
function saveShipment() { const clientName = document.getElementById("shipClientName").value, clientPhone = document.getElementById("shipClientPhone").value, clientAddress = document.getElementById("shipClientAddress").value, receiverName = document.getElementById("shipReceiverName").value, receiverPhone = document.getElementById("shipReceiverPhone").value, receiverAddress = document.getElementById("shipReceiverAddress").value; let trackingCode = document.getElementById("shipTrackingCode").value.trim(); if (!trackingCode) trackingCode = "SHIP" + Math.floor(Math.random() * 1000000); const notes = document.getElementById("shipNotes").value, imageFile = document.getElementById("packageImage").files[0]; if (!clientName || !receiverName) { alert("Client name and receiver name are required."); return; } let imageData = null; const saveData = () => { const shipment = { id: Date.now(), trackingCode, client: { name: clientName, phone: clientPhone, address: clientAddress }, receiver: { name: receiverName, phone: receiverPhone, address: receiverAddress }, notes, image: imageData, status: "pending", paid: false, date: new Date().toISOString() }; let shipmentsArray = JSON.parse(localStorage.getItem('shipments') || '[]'); shipmentsArray.push(shipment); localStorage.setItem('shipments', JSON.stringify(shipmentsArray)); alert("Shipment saved."); clearShippingForm(); loadShipments(); }; if (imageFile) { const reader = new FileReader(); reader.onload = e => { imageData = e.target.result; saveData(); }; reader.readAsDataURL(imageFile); } else saveData(); }
...
function loadShipments() { const pendingDiv = document.getElementById("pendingShipmentsList"), unpaidDiv = document.getElementById("unpaidShipmentsList"); if (!pendingDiv || !unpaidDiv) return; let shipmentsArray = JSON.parse(localStorage.getItem('shipments') || '[]'); const pending = shipmentsArray.filter(s => s.status === "pending"), unpaid = shipmentsArray.filter(s => s.paid === false); pendingDiv.innerHTML = pending.map(s => `<div class="shipment-card"><strong><i class="fas fa-truck"></i> Tracking:</strong> ${escapeHtml(s.trackingCode)}<br><strong><i class="fas fa-user"></i> Client:</strong> ${escapeHtml(s.client.name)}<br><strong><i class="fas fa-user-check"></i> Receiver:</strong> ${escapeHtml(s.receiver.name)}<br>${s.image ? `<img src="${s.image}" style="max-width:100px;" loading="lazy">` : ''}<br><button onclick="markAsPaid('${s.trackingCode}')" style="background:#28a745;"><i class="fas fa-money-bill"></i> Mark Paid</button> <button onclick="markAsShippedShipment('${s.trackingCode}')" style="background:#007bff;"><i class="fas fa-check-circle"></i> Mark Shipped</button></div>`).join(''); unpaidDiv.innerHTML = unpaid.map(s => `<div class="shipment-card"><strong><i class="fas fa-truck"></i> Tracking:</strong> ${escapeHtml(s.trackingCode)}<br><strong><i class="fas fa-user"></i> Client:</strong> ${escapeHtml(s.client.name)}<br><strong><i class="fas fa-dollar-sign"></i> Total:</strong> $${s.total || '0'}<br><button onclick="markAsPaid('${s.trackingCode}')" style="background:#28a745;"><i class="fas fa-money-bill"></i> Mark Paid</button></div>`).join(''); }
function markAsPaid(trackingCode) { let shipmentsArray = JSON.parse(localStorage.getItem('shipments') || '[]'); const shipment = shipmentsArray.find(s => s.trackingCode === trackingCode); if (shipment) { shipment.paid = true; localStorage.setItem('shipments', JSON.stringify(shipmentsArray)); loadShipments(); alert("Marked as paid."); } }
function markAsShippedShipment(trackingCode) { let shipmentsArray = JSON.parse(localStorage.getItem('shipments') || '[]'); const shipment = shipmentsArray.find(s => s.trackingCode === trackingCode); if (shipment) { shipment.status = "shipped"; localStorage.setItem('shipments', JSON.stringify(shipmentsArray)); loadShipments(); alert("Marked as shipped."); } }
