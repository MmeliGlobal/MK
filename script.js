...
// ==================== CATEGORY HIERARCHY (your original) ====================
...
async function loadProducts() {
  if (!useSupabase || !supabaseClient) { loadDemoProducts(); return; }
  const { data, error } = await supabaseClient.from('products').select('*').eq('is_active', true);
  if (error) { console.error(error); loadDemoProducts(); }
  else {
    allProducts = data.map(p => ({
      id: p.id, name: p.name, desc: p.description || "", cat: p.cat || "General", subcat: p.subcat || "",
      price: p.price, colors: p.colors || ["Default"], sizeOptions: p.size_options || [{ size: "Standard", price: p.price }],
      mainImage: (p.images && p.images[0]) || "https://via.placeholder.com/300", subImages: p.images ? p.images.slice(1) : [],
...
async function saveProduct(product) {
  if (!useSupabase) { /* local save */ const idx = allProducts.findIndex(p => p.id == product.id); if (idx!==-1) allProducts[idx]=product; else allProducts.push(product); localStorage.setItem('customProducts',JSON.stringify(allProducts)); return product; }
  if (!product.id || product.id === 0) {
    const slug = product.slug || generateSlug(product.name);
    const { data: existing } = await supabaseClient.from('products').select('id').eq('slug', slug).maybeSingle();
    const finalSlug = existing ? `${slug}-${Date.now()}` : slug;
    const { data, error } = await supabaseClient.from('products').insert([{
      name: product.name, description: product.desc, cat: product.cat, subcat: product.subcat, price: product.price,
      colors: product.colors, size_options: product.sizeOptions, images: [product.mainImage, ...(product.subImages || [])],
      is_active: true, slug: finalSlug
    }]).select();
...
async function deleteProductFromDB(id) {
  if (!useSupabase) { allProducts = allProducts.filter(p => p.id != id); localStorage.setItem('customProducts', JSON.stringify(allProducts)); return true; }
...
async function checkUser() {
  if (!useSupabase) {
    ...
    } else { loggedInUser = null; document.getElementById("loginBox").style.display = "block"; document.getElementById("dashboard").style.display = "none"; cart = []; updateCartCount(); renderCart(); }
    return;
  }
  const { data: { user } } = await supabaseClient.auth.getUser();
  if (user) {
    loggedInUser = user;
    const { data: profile } = await supabaseClient.from('profiles').select('full_name, is_admin').eq('id', user.id).single();
...
async function login() {
  ...
  const { error } = await supabaseClient.auth.signUp({ email: tempEmail, password: tempPassword, options: { data: { full_name: `${name} ${surname}`, phone, address } } });
  if (error) {
...
async function loadCartFromDB() {
  ...
  const { data: cartRecord } = await supabaseClient.from('carts').select('id').eq('user_id', loggedInUser.id).maybeSingle();
  if (!cartRecord) return;
  const { data: items } = await supabaseClient.from('cart_items').select('*, product_variants(price, product_id, products(name, images))').eq('cart_id', cartRecord.id);
  if (items) {
    cart = items.map(item => ({
      id: item.product_variants.product_id, name: item.product_variants.products.name, price: item.product_variants.price,
      quantity: item.quantity, variant_id: item.variant_id, cart_item_id: item.id, image: item.product_variants.products.images?.[0] || "",
...
async function addToCartDB(product, selectedSize, selectedColor) {
  if (!useSupabase) {
    cart.push({ id: product.id, name: product.name, size: selectedSize || "Standard", color: selectedColor || "Default", price: product.price, image: product.mainImage });
    localStorage.setItem('cart', JSON.stringify(cart)); updateCartCount(); renderCart(); alert("Added to cart"); return;
  }
  if (!loggedInUser) { alert("Please sign in first."); switchPage("account"); return; }
  let variantId;
  const { data: variants } = await supabaseClient.from('product_variants').select('id').eq('product_id', product.id).limit(1);
  if (variants && variants.length) variantId = variants[0].id;
  else {
    const { data: newVariant } = await supabaseClient.from('product_variants').insert({ product_id: product.id, sku: `SKU-${product.id}`, price: product.price, stock_quantity: 999 }).select();
    variantId = newVariant[0].id;
  }
  let { data: cartRecord } = await supabaseClient.from('carts').select('id').eq('user_id', loggedInUser.id).maybeSingle();
  if (!cartRecord) {
    const { data: newCart } = await supabaseClient.from('carts').insert({ user_id: loggedInUser.id }).select();
    cartRecord = newCart[0];
  }
  const { data: existing } = await supabaseClient.from('cart_items').select('id, quantity').eq('cart_id', cartRecord.id).eq('variant_id', variantId).maybeSingle();
  if (existing) await supabaseClient.from('cart_items').update({ quantity: existing.quantity + 1 }).eq('id', existing.id);
  else await supabaseClient.from('cart_items').insert({ cart_id: cartRecord.id, variant_id: variantId, quantity: 1 });
  await loadCartFromDB();
  alert("Added to cart");
}
function addToCart() {
  if (!currentProduct) return;
  const sizeSelect = document.getElementById("sizeSelect"), selectedOption = sizeSelect.options[sizeSelect.selectedIndex];
  const sizeText = selectedOption.text.split(" - ")[0], color = document.getElementById("colorSelect") ? document.getElementById("colorSelect").value : "Default";
  addToCartDB(currentProduct, sizeText, color);
}
async function removeFromCart(index) {
  const item = cart[index];
  if (useSupabase && item.cart_item_id) await supabaseClient.from('cart_items').delete().eq('id', item.cart_item_id);
  cart.splice(index, 1);
  localStorage.setItem('cart', JSON.stringify(cart));
  updateCartCount(); renderCart();
...
async function checkout() {
  ...
  const trackingCode = "MM" + Math.floor(Math.random() * 1000000), total = cart.reduce((sum, i) => sum + i.price, 0);
  if (useSupabase) {
    const { data: order, error } = await supabaseClient.from('orders').insert({ user_id: loggedInUser.id, status: 'pending', total_amount: total, shipping_address: { address: "From profile" } }).select();
    if (error) { alert("Order failed: " + error.message); return; }
    const orderId = order[0].id;
    for (const item of cart) await supabaseClient.from('order_items').insert({ order_id: orderId, product_name: item.name, quantity: item.quantity || 1, price: item.price });
    const { data: cartRecord } = await supabaseClient.from('carts').select('id').eq('user_id', loggedInUser.id).single();
    if (cartRecord) await supabaseClient.from('cart_items').delete().eq('cart_id', cartRecord.id);
  } else {
    const order = { id: Date.now(), items: [...cart], total, user: loggedInUser, trackingCode, status: "Processing", date: new Date().toISOString() };
  ...
  itemsForMsg.forEach(item => { whatsappMsg += `${escapeHtml(item.name)} | ${escapeHtml(item.color)} | ${escapeHtml(item.size)} | 1 | $${item.price.toFixed(2)}%0A`; });
...
function switchPage(pageId) {
  ...
  else if (pageId === "adminInventoryPage") loadInventoryPage();
  else if (pageId === "adminDiscountsPage") loadDiscountsPage();
  else if (pageId === "adminUsersPage") loadUsersPage();
  else if (pageId === "adminAnalyticsPage") loadAnalyticsPage();
...
async function addProduct() { const name = document.getElementById("newName").value, price = parseFloat(document.getElementById("newPrice").value), image = document.getElementById("newImage").value, extraImages = document.getElementById("newExtraImages").value, desc = document.getElementById("newDesc").value, category = document.getElementById("newCategory").value, subcategory = document.getElementById("newSubcategory").value, colorInput = document.getElementById("newColor").value, sizeInput = document.getElementById("newSize").value; if (!name || isNaN(price) || !image || !category || !subcategory) { alert("Please fill required fields."); return; } let sizeOptions = []; if (sizeInput) { const pairs = sizeInput.split(','); pairs.forEach(pair => { const [size, p] = pair.split(':'); if (size && p) sizeOptions.push({ size: size.trim(), price: parseFloat(p) }); }); } if (sizeOptions.length === 0) sizeOptions.push({ size: "Standard", price: price }); let colors = colorInput ? colorInput.split(',').map(c => c.trim()) : ["Default"]; let subImages = extraImages ? extraImages.split(',').map(url => url.trim()).filter(u => u) : []; const newProduct = { name, slug: generateSlug(name), desc: desc || "Added by admin", cat: category, subcat: subcategory, price, colors, sizeOptions, mainImage: image, subImages }; const inserted = await saveProduct(newProduct); if (inserted) { allProducts.push(inserted); allProducts = shuffleArray(allProducts); displayHomeProducts(allProducts); loadAdminSummaries(); loadProductsFull(); alert("Product added!"); } else alert("Failed to add product. Check console for error."); document.getElementById("newName").value = ""; document.getElementById("newPrice").value = ""; document.getElementById("newImage").value = ""; document.getElementById("newExtraImages").value = ""; document.getElementById("newDesc").value = ""; document.getElementById("newCategory").value = ""; document.getElementById("newSubcategory").value = ""; document.getElementById("newColor").value = ""; document.getElementById("newSize").value = ""; switchPage("adminDashboard"); }
...
async function updateProduct() { const name = document.getElementById("newName").value, price = parseFloat(document.getElementById("newPrice").value), image = document.getElementById("newImage").value, extraImages = document.getElementById("newExtraImages").value, desc = document.getElementById("newDesc").value, category = document.getElementById("newCategory").value, subcategory = document.getElementById("newSubcategory").value, colorInput = document.getElementById("newColor").value, sizeInput = document.getElementById("newSize").value; if (!name || isNaN(price) || !image || !category || !subcategory) { alert("Please fill required fields."); return; } let sizeOptions = []; if (sizeInput) { const pairs = sizeInput.split(','); pairs.forEach(pair => { const [size, p] = pair.split(':'); if (size && p) sizeOptions.push({ size: size.trim(), price: parseFloat(p) }); }); } if (sizeOptions.length === 0) sizeOptions.push({ size: "Standard", price: price }); let colors = colorInput ? colorInput.split(',').map(c => c.trim()) : ["Default"]; let subImages = extraImages ? extraImages.split(',').map(url => url.trim()).filter(u => u) : []; const updatedProduct = { id: editProductId, name, slug: generateSlug(name), desc: desc || "Added by admin", cat: category, subcat: subcategory, price, colors, sizeOptions, mainImage: image, subImages }; const saved = await saveProduct(updatedProduct); if (saved) { const index = allProducts.findIndex(p => p.id == editProductId); if (index !== -1) allProducts[index] = saved; allProducts = shuffleArray(allProducts); displayHomeProducts(allProducts); loadAdminSummaries(); loadProductsFull(); alert("Product updated."); } else alert("Update failed."); document.getElementById("newName").value = ""; document.getElementById("newPrice").value = ""; document.getElementById("newImage").value = ""; document.getElementById("newExtraImages").value = ""; document.getElementById("newDesc").value = ""; document.getElementById("newCategory").value = ""; document.getElementById("newSubcategory").value = ""; document.getElementById("newColor").value = ""; document.getElementById("newSize").value = ""; const addBtn = document.querySelector("#adminAddProductPage button"); addBtn.innerHTML = '<i class="fas fa-plus"></i> Add Product'; addBtn.onclick = () => addProduct(); editProductId = null; switchPage("adminDashboard"); }
...
// ==================== QUOTATION FUNCTIONS (keep your existing) ====================
function setupQuotationForm() { /* your existing code – unchanged */ }
function addItemRow() { /* your existing code */ }
function calculateQuoteTotals() { /* your existing code */ }
function generateQuote() { /* your existing code */ }
function viewQuote(quoteId) { /* your existing code */ }
function closeQuoteModal() { /* your existing code */ }
function printQuote() { /* your existing code */ }

// ==================== SHIPPING FUNCTIONS (keep your existing) ====================
function saveShipment() { /* your existing code */ }
function clearShippingForm() { /* your existing code */ }
function loadShipments() { /* your existing code */ }
function markAsPaid(trackingCode) { /* your existing code */ }
function markAsShippedShipment(trackingCode) { /* your existing code */ }

// ==================== NEW ADMIN PAGES ====================
function loadInventoryPage() { const container = document.getElementById("inventoryList"); if (!container) return; if (!useSupabase) { container.innerHTML = "<p>Inventory management requires Supabase.</p>"; return; } supabaseClient.from('product_variants').select('*, products(name)').then(({ data }) => { container.innerHTML = `<table border="1" cellpadding="8"><table><th>Product</th><th>SKU</th><th>Stock</th><th>Low Threshold</th><th>Action</th></tr>${data.map(v => `<tr><td>${v.products.name}</td><td>${v.sku}</td><td>${v.stock_quantity}</td><td>${v.low_stock_threshold || 5}</td><td><button onclick="updateStock('${v.id}')"><i class="fas fa-edit"></i> Update</button></td></tr>`).join('')}</table>`; }); }
function loadDiscountsPage() { const container = document.getElementById("discountsList"); if (!container) return; if (!useSupabase) { container.innerHTML = "<p>Discounts require Supabase.</p>"; return; } supabaseClient.from('discounts').select('*').then(({ data }) => { container.innerHTML = `<button onclick="showAddDiscountForm()"><i class="fas fa-plus"></i> Add Discount</button><table border="1" cellpadding="8"><tr><th>Code</th><th>Type</th><th>Value</th><th>Active</th><th>Actions</th></tr>${data.map(d => `<tr><td>${d.code}</td><td>${d.discount_type}</td><td>${d.value}</td><td>${d.is_active ? '✅' : '❌'}</td><td><button onclick="toggleDiscount('${d.id}')"><i class="fas fa-toggle-on"></i> Toggle</button></td></tr>`).join('')}</table>`; }); }
function loadUsersPage() { const container = document.getElementById("usersList"); if (!container) return; if (!useSupabase) { container.innerHTML = "<p>User management requires Supabase.</p>"; return; } supabaseClient.from('profiles').select('*').then(({ data }) => { container.innerHTML = `<table border="1" cellpadding="8"><tr><th>Email</th><th>Full Name</th><th>Phone</th><th>Admin</th><th>Action</th></tr>${data.map(u => `<tr><td>${u.email}</td><td>${u.full_name || ''}</td><td>${u.phone || ''}</td><td>${u.is_admin ? 'Yes' : 'No'}</td><td><button onclick="toggleAdmin('${u.id}')"><i class="fas fa-user-shield"></i> Toggle Admin</button></td></tr>`).join('')}</table>`; }); }
function loadAnalyticsPage() { const container = document.getElementById("analyticsContent"); if (!container) return; if (!useSupabase) { container.innerHTML = "<p>Analytics require Supabase.</p>"; return; } supabaseClient.from('orders').select('total_amount, created_at').then(({ data }) => { const totalSales = data.reduce((sum, o) => sum + o.total_amount, 0); const orderCount = data.length; container.innerHTML = `<p>Total Orders: ${orderCount}</p><p>Total Sales: $${totalSales.toFixed(2)}</p><p>Average Order Value: $${(totalSales/orderCount || 0).toFixed(2)}</p>`; }); }
function updateStock(variantId) { const newStock = prompt("Enter new stock quantity:"); if (newStock !== null) supabaseClient.from('product_variants').update({ stock_quantity: parseInt(newStock) }).eq('id', variantId).then(() => loadInventoryPage()); }
function showAddDiscountForm() { alert("Add discount form – implement as needed."); }
function toggleDiscount(discountId) { supabaseClient.from('discounts').select('is_active').eq('id', discountId).single().then(({ data }) => { supabaseClient.from('discounts').update({ is_active: !data.is_active }).eq('id', discountId).then(() => loadDiscountsPage()); }); }
function toggleAdmin(userId) { supabaseClient.from('profiles').select('is_admin').eq('id', userId).single().then(({ data }) => { supabaseClient.from('profiles').update({ is_admin: !data.is_admin }).eq('id', userId).then(() => loadUsersPage()); }); }
