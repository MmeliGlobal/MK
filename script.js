...
// ==================== DIRECT REST API FOR PRODUCTS (BYPASSES SCHEMA CACHE) ====================
async function insertProductDirect(product) {
...
async function updateProductDirect(productId, product) {
...
async function saveProduct(product) {
  ...
  if (!product.id || product.id === 0) {
    const newProduct = await insertProductDirect(product);
    // Create default variant
    const variantSku = `${newProduct.slug}-DEFAULT`;
    ...
    }).catch(e => console.warn("Variant creation failed", e));
    return newProduct;
  } else {
    const updated = await updateProductDirect(product.id, product);
...
async function addProduct() {
  ...
  let finalSlug = slug;
  let counter = 1;
  while (allProducts.some(p => p.slug === finalSlug)) {
...
// ==================== QUOTATION FUNCTIONS (preserve your existing) ====================
...
// ==================== SHIPPING FUNCTIONS (preserve your existing) ====================
